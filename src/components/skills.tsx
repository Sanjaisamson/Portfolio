...
export const Skills = () => {
  useEffect(() => {
    // Initialize GA4 only once when the component mounts
  ...
  const projectCardData = [
    ...
    { name: "REACT-Native", logo: react_native_logo },
      ...
      <div className='skills_glass_box'>
        ...
        <div className='skills_glass_box_cards'>
          {projectCardData.map((card, index) => (
            <Card key={index} name={card.name} logo={card.logo}/>
          ))}
        </div>
      </div>
    </div>
